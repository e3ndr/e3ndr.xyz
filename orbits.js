...
function draw(e) {
    ...
    for (let i = 0; i < count; i++) {
        var _movement$i, _velocities$i;

        const randMovement = (_movement$i = movement[i]) !== null && _movement$i !== void 0 ? _movement$i : movement[i] = 0;
        const velocity = (_velocities$i = velocities[i]) !== null && _velocities$i !== void 0 ? _velocities$i : velocities[i] = getRandomArbitrary(minVelocity, maxVelocity);
        ...
        const v = Vector.fa(a, r).setZ(z).addXY(randMovement, randMovement).rotate(time * velocity).rotateYZ(15);
        v.div((v.z + 800) / 800);
        circle(v, 1);
    }
